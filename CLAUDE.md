...
- **Bronze** — raw, untouched Airbyte output. One schema per source: `BRONZE_SHOPIFY_<BRAND>`, `BRONZE_KLAVIYO_<BRAND>`, `BRONZE_META_<BRAND>`, `BRONZE_GOOGLE_ANALYTICS_<BRAND>`, `BRONZE_GOOGLE_ADS_<BRAND>`, `BRONZE_CIN7`, `BRONZE_PROSPECT_CRM`. Only Airbyte writes here.
...
- **Schemas:** `BRONZE_SHOPIFY_<BRAND>` · `BRONZE_KLAVIYO_<BRAND>` · `BRONZE_META_<BRAND>` · `BRONZE_GOOGLE_ANALYTICS_<BRAND>` · `BRONZE_GOOGLE_ADS_<BRAND>` · `BRONZE_CIN7` · `BRONZE_PROSPECT_CRM` · `SILVER` · `GOLD` · `METRICS`
...
- **Meta audience-type classification** — `audience_type` on `stg_meta__ads_spend` / `fct_ad_spend` is a name-pattern classification of the Meta campaign + ad-set name (it is not a native Meta field). The classifier upper-cases `campaign_name || ' ' || adset_name` and matches substrings: `REMARKETING` / `RETARGETING` / `EXISTING CUSTOMERS` → `remarketing`; `PROSPECTING` / `ACQUISITION` / `LOOKALIKE` → `acquisition`; everything else → `other` (dominated by the Skin Quiz `OUTCOME_LEADS` lead campaign). Remarketing is tested before acquisition, so a campaign carrying both signals lands in remarketing. This regrains `fct_ad_spend` to `spend_date` × `store_id` × `audience_type`; any per-day or per-brand consumer must re-aggregate. When a new ad account is onboarded, confirm its naming convention matches these substrings or extend the classifier.
- **Google Ads `brand_intent` classification** — `brand_intent` on `stg_google_ads__search_terms` / `fct_google_ads_search_terms` is a name-pattern classification of the Google Ads search-term text (it is not a native Google Ads field). A term is `branded` if its lower-cased text contains the brand name (`is clinical` / `isclinical` / `is-clinical`), a common truncation or misspelling (`is clini` / `isclinic` / `is clin` / `es clinical` / `is skin`), or an iS Clinical product name (`cleansing complex`, `pro heal serum`, `hydra cool serum`, `active serum`, `super serum`, `sheald`, `youth eye complex`, etc.); everything else is `non_branded` (generic acquisition intent). Source: `BRONZE_GOOGLE_ADS_ISCLINICAL.ADS_SEARCHQUERYSTATS_5994178388` (Airbyte's search-query stream; `5994178388` is the iS Clinical ad-account id, the `P_ADS_*` variant is the partitioned table). `fct_google_ads_search_terms` is scoped to the top 100 search terms by cost per month, aggregated to `(order_month, brand_intent)`. Coverage caveat: Google Ads' search-term view only retains terms above a minimum impression threshold, so Bronze holds fewer search terms than a UI "search term raw report" exported at report time. When a new brand's Google Ads account is onboarded, add a `BRONZE_GOOGLE_ADS_<BRAND>` source and extend the union the same way the Shopify/Klaviyo conventions describe.
- **GA4 `channel` classification.** `channel` on `fct_ga_source_medium` is a name-pattern classification of the GA4 `session_source` / `session_medium` into the same seven channels `fct_ga_channel_sessions` uses (`Direct`, `Organic Search`, `Paid Search`, `Paid Social`, `Email / CRM`, `Referral / Affiliates`, `Other`). It is not a native GA4 field: the source/medium Bronze report carries no channel grouping (only the default-channel-grouping report behind `fct_ga_channel_sessions` does), so it is reconstructed from source/medium text. It is declared as a Lightdash `additional_dimensions` entry on the `source_medium` column in `dbt/models/gold/_schema.yml` (query-time SQL over `session_source` / `session_medium`), not a warehouse column like `audience_type` / `brand_intent`. That is deliberate: an additional dimension ships the moment `lightdash deploy` runs, whereas a real column would lag behind slim CI until the next daily dbt run and briefly break any chart referencing it. Powers the iS Clinical Attribution Sankey (`lightdash/dashboards/isclinical-attribution.yml`). When a new brand's GA4 connection lands, check its source/medium values against the classifier substrings and extend them.
- **Email normalisation** — `LOWER(TRIM(email))` in Silver before joining Shopify customers to Klaviyo profiles.
- **Klaviyo CRM revenue attribution** — Klaviyo CRM metrics are built from the raw `BRONZE_KLAVIYO_<BRAND>.EVENTS` table, not from Klaviyo's pre-aggregated `*_values_reports` streams (those are deliberately not synced). Event identity: `EVENTS.RELATIONSHIPS:metric:data:id` joins to `METRICS.ID`; the metric name is `METRICS.ATTRIBUTES:name`. There are multiple `METRICS` rows named `Placed Order` per account (one per integration, e.g. native Shopify + server-side API), so each order fires several `Placed Order` events. **Deduplicate Placed Orders by `(store_id, OrderID)` before summing `$value`** (`stg_klaviyo__placed_orders` does this). Engagement events (`Received`/`Opened`/`Clicked Email`) carry `$campaign`/`$flow`/`$message` in `event_properties` (note: not `$campaign_id`/`$flow_id`); Placed Order events carry `$value`/`$value_currency`/`OrderID` only. **Campaign/flow revenue is a 5-day-window attribution computation**: `fct_klaviyo_revenue` matches each deduped Placed Order to that profile's most recent `Opened`/`Clicked Email` engagement in the 5 days before the order and inherits its campaign or flow. This reconstructs Klaviyo's native attribution from raw events, so a few-percent residual gap against Klaviyo's own engine is expected and accepted (April 2026 iSC: model £67,911 vs PDF £64,885, +4.7%). **Per-campaign rollup:** `fct_klaviyo_campaign_revenue` aggregates `fct_klaviyo_revenue`'s campaign-attributed rows to one row per `campaign_id × store_id × order_month` (it reuses the attribution, never re-runs it). The campaign **send name** is not recoverable from the `CAMPAIGNS` stream: the `$campaign` key on raw events does not join to `CAMPAIGNS.id` (0% overlap, disjoint id spaces). The only usable name source is the `Campaign Name` property carried on the engagement events themselves (one distinct name per `$campaign`), so `fct_klaviyo_campaign_revenue` builds a `campaign_id → campaign_name` lookup from `stg_klaviyo__events`. **Per-flow rollup:** `fct_klaviyo_flow_revenue` does the same for flow-attributed rows (one row per `flow_id × store_id × order_month`). Unlike campaigns, the `$flow` event key **does** join cleanly to the `FLOWS` stream id (verified 2026-05-20, 23/23 distinct event flow ids matched), so the flow name comes from `stg_klaviyo__flows` (a Silver flow dimension off `BRONZE_KLAVIYO_<BRAND>.FLOWS`, 45 iSC rows). Klaviyo flow names are not guaranteed unique (live / draft copies share a name), so `flow_id` is the grain. The flow model inherits the same residual gap against Klaviyo's native engine; in addition the PDF KPI report splits Abandoned Checkout / Abandoned Cart into base + variant rows that this single-row-per-flow reconstruction does not reproduce.
- **Klaviyo open/click rate** — open and click rate need no attribution join: `stg_klaviyo__email_engagement` aggregates raw `Received`/`Opened`/`Clicked Email` events to a monthly grain per `store_id`, and `fct_campaign_performance` (rebuilt off it, since the legacy `campaign_values_reports`-backed version was empty) exposes `delivered`, `unique_opens`, `unique_clicks`. `delivered` = `Received Email` event count; `unique_opens`/`unique_clicks` = distinct `($message, profile)` pairs (counting raw open events overcounts badly due to multi-open inflation and Apple MPP). The Lightdash `avg_open_rate`/`avg_click_rate` metrics are **weighted ratio** number metrics (`sum(unique_opens)/sum(delivered)`) so a filtered tile reads the exact aggregate rate. A small residual gap against Klaviyo's native open rate is expected (April 2026 iSC: model 69.8% vs PDF 67.7%); click rate reconciles near-exactly (1.73% vs 1.72%).
- **Shopify Gross / Net / Total sales convention.** Every $ figure in this project must be classifiable as one of three Shopify chart-of-accounts lines, and labels / tooltips must say which:
  - **Gross sales** = product price × quantity, before any discount, return, tax or shipping. Computed on `fct_orders` as `subtotal_price + total_discounts` (Shopify's `subtotal_price` already has discounts applied, so we add them back).
  - **Net sales** = Gross − discounts − returns; excludes tax and shipping on both sides. Computed on `fct_orders` as `subtotal_price - refund_subtotal`, where `refund_subtotal` is the product-only portion of refund line items (not the full refund transaction, which would also include tax + shipping). `stg_shopify__order_refunds` exposes both `refund_amount` (full) and `refund_subtotal` (product).
  - **Total sales** = customer-paid grand total (= Net + tax + shipping), pre-refund. This is Shopify's `total_price`. The legacy `total_revenue` Lightdash metric is this figure (kept as a back-compat name).
  - Klaviyo `$value`, GA4 purchase-event value and Meta purchase value all mirror Shopify `total_price`, so attributed-revenue metrics from those sources are **closest to Total sales**, not Gross or Net. Their tooltips must say so. Google Ads cost is paid-media spend, not revenue, and is exempt.
  - The KPI Report headlines Gross with Net alongside (via the `comparisonField` on `big_number` tiles). When adding a new $ tile, pick the right Shopify line for the metric, label it explicitly, and add the matching tooltip; do not introduce a new $ metric without this classification.
