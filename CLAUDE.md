...
- **Lightdash builders are reference-only; chart edits go through migrations.** `lightdash/build_dashboards.py` and `lightdash/build_prospect_crm_dashboards.py` were one-shot scripts used to seed the initial dashboards. They are kept in the repo as a historical record of how the dashboards were originally constructed and **must not be re-run** — each call POSTs new charts (no upsert) and would create duplicates. Treat them as read-only documentation. For all subsequent chart/dashboard edits, write a one-shot migration in `lightdash/migrations/` instead. See `lightdash/migrations/README.md` for the pattern.
