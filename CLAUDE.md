...
- **Lightdash PRs must list post-deploy ops in the PR description.** `lightdash deploy` (run by `lightdash_deploy.yml` on push to `main`) only refreshes the dbt project metadata. It does **not** update saved-chart configs (column order, dimensions, eCharts formatters), recreate dashboards from the builders, or delete obsolete dashboards. Any PR that touches `lightdash/build_*.py`, the chart layer of `dbt/models/gold/_schema.yml`, or chart-affecting dbt SQL must include a "Post-deploy ops" section spelling out the exact commands the user needs to run after merge — typically some subset of: deleting stale dashboards in the UI, re-running the relevant `python3 lightdash/build_*.py` script, or curl-PATCHing/-DELETEing specific saved-chart / dashboard UUIDs via the Lightdash API. Default to copy-paste-ready shell commands that read auth from `.env` (`LIGHTDASH_URL`, `LIGHTDASH_TOKEN`).
